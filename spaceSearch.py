...
def generate_state(old_state, n, method, config={}):
    ...
    if method == "gaussian_all":  # generate a state where each directions is a random number generated from a gaussian distributuion around the old number
        mu, sigma = 0, 0.2
        ...
        for i in range(len(res)):
            if res[i] < Min:
                ...
                res[i] = min(Max, n-1)
    elif method == "gaussian_same_all":  # move all directions same amount generated by a gaussian distribution.
        mu, sigma = 0, 0.2
...
class SpaceSearch:
    def __init__(self, ggwr, cv=1):
        '''
        :param ggwr: ggwr model passed to space search
        :param cv: if equal 1 means no cross validation, 20% of data will be considered as validation
        '''
        self.ggwr = ggwr
        self.cs = get_configspace(ggwr.numberOfFeatures, ggwr.train_len)
        indices = np.random.permutation(ggwr.train_len)
        self.validation_indices = [[] for i in range(cv)]
        self.train_indices = [[] for i in range(cv)]
        if cv == 1:
            validationLen = int(0.14 * ggwr.train_len)
            self.validation_indices[0] = indices[:validationLen]
            self.train_indices[0] = indices[validationLen:]
        else:
            validationLen = int(ggwr.train_len / cv)
            for i in range(cv):
    ...
    def hyperband(self, start_configs_count, min_budget, eta):
        ...
        for i in range(smax, -1, -1):
            ...
            print("min_budget", min_budget * (eta ** (smax - i)), "n", n, "max_budget", max_budget)
            ...
            print("min_budget", min_budget * (eta ** (smax - i)), "n", n, "max_budget", max_budget, "best_config",
                  best_config, "best_error", best_error,  list(config.values()))
    ...
    def successive_halving(self, start_configs_count, min_budget, max_budget, eta):
        ...
        while n > 0 and budget <= max_budget:
            for i in range(len(configs)):
                # print(list(configs[i][0].values()), budget)
            ...
            if budget == max_budget:
                ...
                if self.ggwr.config["enable_cache"]:
                    self.ggwr.cache.clear()
        return configs[0]
    ...
    def simulated_annealing(self, starting, config):
        ...
        while step < config["steps"] and updates < config["updates"]:
            new_state = generate_state(state, self.ggwr.train_len, config["method"])
            # print(new_state)
            # if step == 1 and config["method"] == "gaussian_all":
            #     new_state = [878., 862.,  11.,   3. ,  9. ,  3.]
            # start = time.time()
        ...
        # print("sim err", best_error, state)
    ...
    def hill_climbing(self, starting, config):
        ...
        while step < config["steps"] and updates < config["updates"]:
            ...
            step += 1
            temp_error = self.evaluate(new_state, best_error)
            # print(new_state, state, temp_error, best_error)
            if temp_error < best_error:
                state = np.copy(np.asarray(new_state))
                best_error = temp_error
                updates += 1
                ...
                if temp_error < best_error:
                    ...
                    direction = 1
                    optimizing_feature %= self.ggwr.numberOfFeatures
    ...
    def thorough_search(self, starting, config):
        ...
        state = np.copy(np.asarray(starting))
        for i in range(2 ** self.ggwr.numberOfFeatures):
            new_state = [Max if i & (1 << j) else Min for j in range(self.ggwr.numberOfFeatures)]
            temp_error = self.evaluate(new_state, best_error)
            # print(new_state, temp_error, self.ggwr.config["enable_cache"])
            if temp_error < best_error:
                state = np.copy(np.asarray(new_state))
                best_error = temp_error
    ...
    def golden_section(self, starting, config={"steps": 100}):
        ...
        while step < config["steps"]:
            for optimizing_feature in range(self.ggwr.numberOfFeatures):
                a = Min
                b = Max
                c = b - (b - a) / gr
                d = a + (b - a) / gr
    ...
    def bayesian_optimization(self, starting, config={}):
        ...
        pbounds = {}
        if config["is_local"]:
            for _ in range(self.ggwr.numberOfFeatures):
                temp_min = max(Min, starting[_]-config["locality_range"])
                temp_max = min(Max, self.ggwr.train_len-1, starting[_]+config["locality_range"])
                pbounds['bandwidth' + str(_)] = (temp_min, temp_max)
        else:
            for _ in range(self.ggwr.numberOfFeatures):
                pbounds['bandwidth' + str(_)] = (Min, min(Max, self.ggwr.train_len-1))
    ...
    def FDSA(self, starting, config= {}):
        ...
        for iter in range(config["steps"]):
            ...
            for feature in range(len(state)):
                ...
                ta[feature] = min(ta[feature], Max, self.ggwr.train_len - 1)

                tb[feature] = round(tb[feature])
                tb[feature] = max(Min, tb[feature])
                tb[feature] = min(tb[feature], Max, self.ggwr.train_len - 1)
            ...
            for j in range(len(ta)):
                ...
                state[j] = min(state[j], Max, self.ggwr.train_len-1)
            temp_error = self.evaluate(state)
            # print(round(best_error, 5), round(temp_error, 5), np.asarray(temp).astype(int), state.astype(int))
            if temp_error < best_error:
                best_error = temp_error
                best_state = np.copy(np.asarray(state))
    ...
    def SPSA(self, starting, config= {}):
        ...
        for iter in range(config["steps"]):
            ...
            for j in range(len(ta)):
                ta[j] = round(ta[j])
                ta[j] = max(Min, ta[j])
                ta[j] = min(ta[j], Max, self.ggwr.train_len-1)

                tb[j] = round(tb[j])
                tb[j] = max(Min, tb[j])
                tb[j] = min(tb[j], Max, self.ggwr.train_len-1)
            ...
            for j in range(len(ta)):
                ...
                state[j] = min(state[j], Max, self.ggwr.train_len-1)
            temp_error = self.evaluate(state)
            # print(round(best_error, 5), round(temp_error, 5), round(ak, 2), np.asarray(temp).astype(int), state.astype(int))
            if temp_error < best_error:
                best_error = temp_error
                best_state = np.copy(np.asarray(state))
    ...
    def bayesian_optimization_evaluate(self, **bandwidths):
        ...
        for i in range(len(self.validation_indices)):
            self.ggwr.fit(state, self.train_indices[i])
            valid_pred = self.ggwr.validate(state, self.validation_indices[i], self.train_indices[i][0:-1])
            errors[i] = utils.R2(self.ggwr.y_training[self.validation_indices[i]], valid_pred)
    ...
    def evaluate(self, state, best_so_far=None, budget=-1):
        ...
        for i in range(len(self.validation_indices)):
            if budget == -1:
                ...
                self.ggwr.fit(state, self.train_indices[i][0:budget])
            valid_pred = self.ggwr.validate(state, self.validation_indices[i], self.train_indices[i][0:budget])
            errors[i] = utils.R2(self.ggwr.y_training[self.validation_indices[i]], valid_pred)
